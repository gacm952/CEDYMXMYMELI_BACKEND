...
(async () => {
  try {
    const data = {
      ...
      html: '<table style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 10px; box-shadow: 0px 2px 4px rgba(0, 0, 0, 0.1);"><tr><td style="text-align: center; padding: 20px 0; border-top: 5px solid #16a34a;"><p style="font-size: 32px; font-weight: bold; margin: 0 20px;">¡Hola ' + name + '!</p><p style="font-size: 16px; margin: 0 20px;">Tu registro ya casi está listo, solo debes confirmarlo con el siguiente enlace:</p><p style="margin-bottom: 20px;"><a href="' + process.env.FRONTEND_URL + '/ConfirmAccount/' + token + '" style="display: inline-block; background-color: #16a34a; color: #ffffff; font-size: 16px; text-decoration: none; padding: 12px 30px; border-radius: 30px; margin: 20px 0; text-transform: uppercase;">Confirma tu cuenta aquí</a></p><p style="font-size: 14px; margin: 0;">Si tú no creaste esta cuenta, puedes ignorar este mensaje.</p></td></tr></table>',
    };
